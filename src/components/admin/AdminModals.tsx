...
function ExitIntentModal({ onClose }: ModalProps) {
  ...
  function normalizeImageUpload(file: File): Promise<string> {
    const quality = Math.max(
      0.5,
      Math.min(0.92, config.landing.imageOptimization.quality || 0.82),
    );
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        ...
        image.onload = () => {
          ...
          const targetType = config.landing.imageOptimization
          ...
          canvas.toBlob(
            (blob) => {
              ...
              blobReader.onerror = () =>
                reject(new Error("read optimized image failed"));
...
function AnalyticsModal({ onClose }: ModalProps) {
  ...
  const [reportSchedule, setReportSchedule] = useState<string>(
    config.analyticsReport.schedule || "off",
  );
  const [reportNote, setReportNote] = useState<string>(
    "Báo cáo tổng kết hoạt động marketing trong ngày.",
  );
  ...
  useEffect(() => {
    ...
    config.analyticsReport.recipientEmail,
    config.analyticsReport.subject,
    config.analyticsReport.body,
    config.analyticsReport.schedule,
  ]);
  ...
  const handleExport = async (format: "csv" | "json" | "pdf") => {
    ...
    if (format === "csv")
      exportAnalyticsReportCsv(leads, snapshot, {
        start: rangeStart,
        end: rangeEnd,
      });
    if (format === "json")
      exportAnalyticsReportJson(leads, snapshot, {
        start: rangeStart,
        end: rangeEnd,
      });
    if (format === "pdf")
      exportAnalyticsReportPdf(leads, snapshot, {
        start: rangeStart,
        end: rangeEnd,
      });
    setActionMessage(
      `Đã xuất báo cáo ${summary.totalLeads} leads trong khoảng ${rangeStart} → ${rangeEnd}.`,
    );
  };
  ...
  const handleSendReport = async () => {
    ...
    if (!recipients.length) {
      setReportStatus(
        "Chưa có email nhận báo cáo. Điền ít nhất 1 địa chỉ email.",
      );
      return;
    }
    ...
    try {
      ...
      const formattedBody = renderAnalyticsReportTemplate(
        reportBody ||
          "Báo cáo ngày {date}\n\nLượt truy cập: {totalVisits}\nLượt đăng ký: {totalLeads}\nTỷ lệ CR: {conversionRate}\n\n{note}",
        {
          date: rangeEnd,
      ...
      setReportStatus(
        `Đã gửi báo cáo đến ${payload.recipients ?? recipients.length} email.`,
      );
    } catch {
      setReportStatus(
        "Gửi báo cáo thất bại. Kiểm tra cấu hình Resend / REPORT_RECIPIENT_EMAIL.",
      );
    }
  };
  ...
  const todayCr =
    todayStats.visits > 0
      ? ((todayStats.leads / todayStats.visits) * 100).toFixed(1)
      : "0.0";
  return (
    <AdminModal
    ...
    >
      ...
      <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 dark:border-amber-500/25 dark:bg-amber-500/5">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <Stat
            label="Lead hôm nay"
            value={todayStats.leads.toString()}
            tone="text-emerald-600"
          />
      ...
      <div className="mt-4 rounded-xl border border-neutral-200 bg-neutral-50 p-3 dark:border-white/10 dark:bg-white/5">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <button
            ...
            className="rounded-md border border-neutral-300 px-2 py-1.5 text-[11px] font-semibold"
          >
            Xuất CSV
          </button>
          <button
      ...
      <div className="mt-4 rounded-xl border border-sky-200 bg-sky-50 p-3 dark:border-sky-500/20 dark:bg-sky-500/5">
        ...
        <label className="mt-2 block text-[11px] text-neutral-600">
          ...
          <select
            ...
            className="mt-1 w-full rounded-md border border-sky-200 bg-white px-2 py-2 text-xs"
          >
            {(["off", "2h", "4h", "6h", "12h", "daily", "weekly"] as const).map(
              (option) => (
        ...
        {reportStatus && (
          <p className="mt-2 text-[11px] text-sky-700">{reportStatus}</p>
        )}
      </div>
...
function VisitorSessionHistoryModal({ onClose }: ModalProps) {
  ...
  const handleDeleteSelected = async () => {
    ...
    setMessage(
      ok
        ? `Đã xoá ${selectedIds.length} phiên đang chọn.`
        : "Không xoá được các phiên đã chọn.",
    );
    if (ok) {
      setSelectedIds([]);
      await refresh();
  ...
  const handleClearAll = async () => {
    ...
    setMessage(
      ok ? "Đã xoá toàn bộ lịch sử phiên." : "Không xoá được lịch sử phiên.",
    );
    if (ok) {
      setSelectedIds([]);
      await refresh();
          ...
          {filteredSessions.map((session) => (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2 text-[11px] text-neutral-600">
                ...
                <div>
                  <strong>Visitor:</strong> {session.visitorId}
                </div>
                <div>
                  <strong>Source:</strong> {session.source || "direct"}
                </div>
                <div>
                  <strong>Medium:</strong> {session.medium || "-"}
                </div>
                <div>
                  <strong>Campaign:</strong> {session.campaign || "-"}
                </div>
                <div>
                  <strong>Content:</strong> {session.content || "-"}
                </div>
                <div>
                  <strong>Device:</strong> {session.deviceKind || "-"}
                </div>
                <div>
                  <strong>Model:</strong> {session.deviceModel || "-"}
                </div>
                <div>
                ...
                <div>
                  <strong>Variant:</strong> {session.variant || "A"}
                </div>
                <div>
                  <strong>Network:</strong> {session.network || "-"}
                </div>
                <div>
                  <strong>Vị trí:</strong> {session.city || "-"}
                </div>
                <div>
                  <strong>Quốc gia:</strong> {session.country || "-"}
                </div>
                <div>
                  <strong>Session #:</strong> {session.currentSession}
                </div>
                <div>
                  <strong>Visits/day:</strong> {session.todayVisits}
                </div>
              </div>
            </div>
          ))}
